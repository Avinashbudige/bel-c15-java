...
if [ "$DB_TYPE" = "h2" ]; then
	...
	if [ ! -f "$H2_JAR" ]; then
		...
		wget -q -O "$H2_JAR" https://repo1.maven.org/maven2/com/h2database/h2/2.2.222/h2-2.2.222.jar || {
	...
	java -cp "$OUT_DIR:$H2_JAR" org.h2.tools.RunScript -url "jdbc:h2:./data/parking;MODE=PostgreSQL;DB_CLOSE_DELAY=-1" -user sa -script "$ROOT_DIR/db/migration/V1__create_schema_h2.sql"
